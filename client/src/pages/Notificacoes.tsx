...
import { Loader2, Bell, BellOff, Trophy, AlertTriangle, Calendar, Check } from "lucide-react";
import { cn } from "@/lib/utils";
import { formatDistanceToNow } from "date-fns";
import { ptBR } from "date-fns/locale";

const tipoConfig: Record<string, { icon: React.ComponentType<{ className?: string }>; color: string; label: string }> = {
  conquista: { icon: Trophy, color: "text-yellow-500 bg-yellow-50", label: "Conquista" },
  alerta_meta: { icon: AlertTriangle, color: "text-orange-500 bg-orange-50", label: "Alerta" },
  lembrete_metricas: { icon: Calendar, color: "text-blue-500 bg-blue-50", label: "Lembrete" },
  ranking: { icon: Trophy, color: "text-purple-500 bg-purple-50", label: "Ranking" },
};

export default function Notificacoes() {
  const utils = trpc.useUtils();
  const { data: notificacoes, isLoading } = trpc.gamificacao.myNotificacoes.useQuery({ apenasNaoLidas: false });
  ...
  const handleMarkAllRead = () => {
    notificacoes?.filter(n => n.lida === "nao").forEach(n => {
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <h4 className={cn("font-semibold", isUnread ? "text-slate-900" : "text-slate-600")}>
                            {notif.titulo}
                          </h4>
                          {isUnread && (
                        ...
                        <p className={cn("text-sm mb-2", isUnread ? "text-slate-600" : "text-slate-500")}>
                          {notif.mensagem}
                        </p>
